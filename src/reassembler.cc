...
void Reassembler::insert_into_buffer( uint64_t first_index, string&& data, bool is_last_substring )
{
  ...
  uint64_t const end_index = first_index + data.size();
  ...
  for ( auto it = buffer_.begin(); it != buffer_.end() && begin_index < end_index; ) {
    ...
    uint64_t const right_index = std::min( it->first, end_index );
    auto len = right_index - begin_index;
    bytes_pushed_ += len;
    buffer_.emplace( it, begin_index, std::move( data.substr(begin_index - first_index,len)) );
